...
1. 发自肺腑的总结
   ...
   - 之前看源码的时候，大家都在讲三级缓存，大概也知道是干什么用的，于是这次就实验一下，不用三级缓存能不能解决依赖的问题。
     ...
     - 定义两个map
       ...
       如果系统发现需要创建代理对象，这时候两个map也搞不定了。因为没有填充完的bean和代理对象放在一个map，key是一样的，无法确保原bean覆盖代理对象还是代理对象覆盖原bean，如果先有原bean，然后生成代理对象，直接把原bean覆盖没了，代理对象也没办法生成，如果是先有的代理对象，则原bean生成完了就不是代理对象了。
